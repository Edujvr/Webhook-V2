...
app.post("/webhook",async(req, res) =>{   
  ...
  //console.log(req.body.queryResult.outputContexts)
	...
	if(action == "broadcasting"){
		...
		var query  = Facebooks.where({ EMAIL : "palessi@pichincha.com"});
		let conteo = 0;
		query.find(function (err, facebook){
			...
			if (err) {
				...
				for(var i=0; i < 1; i++){
					...
					var msg = "¡Felicitaciones "+facebook[i].NOMBRE+" has completado el curso '3 Líneas de Responsabilidad'!, La nota con la que aprobaste el curso es '" +facebook[i].NOTA+ "' \nContamos con tu compromiso en la aplicación de los conocimientos impartidos."	
